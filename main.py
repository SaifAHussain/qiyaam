...
def adjust_for_DST(utc_time, year=None, month=None, day=None):
    ...
    if type(utc_time) == dict:
        ...
        for key, value in utc_time.items():
            if key in ["month", "day"]:
                continue
            utc_time[key] = adjust_for_DST(value, year, utc_time["month"], utc_time["day"])

        return utc_time
    if type(utc_time) == list:
        utc_time = utc_time.copy()
        for i, item in enumerate(utc_time):
            utc_time[i] = adjust_for_DST(item, year)
...
def get_today(location: str):
    ...
    return get_day(location, today.month, today.day)

# Specific time today
@app.get("/{location}/today/{time}")
def get_time_today(location: str, time: str):
    today = dt.datetime.now(dt.UTC).astimezone(uk)
    return get_time(location, today.month, today.day, time)

# Tomorrow
@app.get("/{location}/tomorrow")
def get_tomorrow(location: str):
    tomorrow = dt.datetime.now(dt.UTC).astimezone(uk) + dt.timedelta(days=1)
    return get_day(location, tomorrow.month, tomorrow.day)

# Specific time tomorrow
@app.get("/{location}/tomorrow/{time}")
def get_time_tomorrow(location: str, time: str):
    tomorrow = dt.datetime.now(dt.UTC).astimezone(uk) + dt.timedelta(days=1)
    return get_time(location, tomorrow.month, tomorrow.day, time)
...
def get_yesterday(location: str):
    ...
    return get_day(location, yesterday.month, yesterday.day)

# Specific time yesterday
@app.get("/{location}/yesterday/{time}")
def get_time_yesterday(location: str, time: str):
    yesterday = dt.datetime.now(dt.UTC).astimezone(uk) - dt.timedelta(days=1)
    return get_time(location, yesterday.month, yesterday.day, time)

# Whole Year
@app.get("/{location}")
def get_year(location: str):
    return adjust_for_DST(prayer_times[location.lower()])

# Whole Month
@app.get("/{location}/{month}")
def get_month(location: str, month: int):
    return adjust_for_DST(prayer_times[location.lower()][month-1])

# Whole Day
@app.get("/{location}/{month}/{day}")
def get_day(location: str, month: int, day: int):
    return adjust_for_DST(prayer_times[location.lower()][month-1][day-1])

# Specific time in day
@app.get("/{location}/{month}/{day}/{time}")
def get_time(location: str, month: int, day: int, time: str):
    return {
        time.lower(): adjust_for_DST(prayer_times[location.lower()][month-1][day-1][time.lower()], None, month, day)
    }
