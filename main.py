...
PRAYER_LABELS = Literal[
    "imsaak", "dawn", "sunrise", "noon", "sunset", "maghrib", "midnight"
]
...
class DateModel(BaseModel):
    ...
    year: conint(ge=1900, le=3000) = Field(
...
def adjust_for_DST(utc_time, use_24_hour: bool, year=None, month=None, day=None):
    ...
    if isinstance(utc_time, str):
        ...
        dt_object = dt.datetime.strptime(utc_time, "%H:%M").replace(
            year=year, month=month, day=day, tzinfo=dt.UTC
        )
    ...
    if isinstance(utc_time, dict):
        ...
        for key, value in utc_time.items():
            if key in ["month", "day"]:
                continue
            utc_time[key] = adjust_for_DST(
                value, use_24_hour, year, utc_time["month"], utc_time["day"]
            )
        return utc_time
...
def get_prayer_datetime(
    location: str, year: int, month: int, day: int, prayer: str
) -> dt.datetime:
    ...
    return time_obj.replace(year=year, month=month, day=day, tzinfo=dt.UTC).astimezone(
        uk
    )
...
def validate_day(month: int, day: int) -> None:
    ...
    if not 1 <= day <= days_in_month:
        raise HTTPException(
            status_code=400,
            detail=f"Day must be between 1 and {days_in_month} for month {month}",
        )
...
def validate_prayer_label(time: str) -> str:
    ...
    if time not in VALID_PRAYER_LABELS:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid prayer time '{time}'. Valid options: {', '.join(VALID_PRAYER_LABELS)}",
        )
    return time
...
def get_time(location: str, month: int, day: int, time: str, use_24_hour: bool = True):
    ...
    return {
        time: adjust_for_DST(
            prayer_times[location][month - 1][day - 1][time],
            use_24_hour,
            None,
            month,
            day,
